...
def main(
    ...
    logger.info(
        'Processing FASTQs {} and {} with Sample ID {} using '
    ...
    if 'mapping' in STEPS:
        ...
        cmd = '{} --cores {} --fastqc --paired --basename sample {} {}'.format(
            TRIMGALORE, THREADS, R1, R2
        )
        exec_command(cmd)
        ...
        cmd = (
            '{} --genomeDir {} --readFilesIn sample_val_1.fq.gz sample_val_2.fq.gz --outSAMorder Paired'
        ...
        cmd = (
            '{} AddOrReplaceReadGroups --INPUT Aligned.sortedByCoord.out.bam --OUTPUT sample_header.bam '
            '--SORT_ORDER coordinate --RGID {} --RGPL Illumina --RGLB DNA --RGPU {} --RGSM {} --RGCN {} '
            '--CREATE_INDEX true --VALIDATION_STRINGENCY SILENT'.format(
                PICARD, SAMPLEID, SAMPLEID, SAMPLEID, SAMPLEID
            )
        )
        exec_command(cmd)
    ...
    if 'gatk' in STEPS:
        ...
        if SPARK:
            cmd = '{} --java-options "-Xmx32g" MarkDuplicatesSpark -I sample_header.bam -O sample_dedup.bam'.format(
                GATK
            )

        else:
            cmd = (
                '{} --java-options "-Xmx32g" MarkDuplicates -I sample_header.bam -O sample_dedup.bam '
        ...
        cmd = '{} SplitNCigarReads --reference {} --input sample_dedup.bam --output sample_split.bam'.format(
            GATK, GENOME
        )
        exec_command(cmd)
        ...
        cmd = (
            '{} --java-options "-Xmx32g" {} --use-original-qualities --input sample_split.bam --reference {} --known-sites {} '
            '--known-sites {} --known-sites {} --output sample_recal_data.txt'.format(
                GATK, recal_cmd, GENOME, SNPSITES, KNOWN_SITE1, KNOWN_SITE2
            )
        )
        exec_command(cmd)
        cmd = (
            '{} ApplyBQSR --use-original-qualities --add-output-sam-program-record --reference {} --input sample_split.bam '
        ...
        # BamQC
        cmd = (
            '{} -bam sample_final.bam -gtf {} --paired -outdir bamQCRNA '
            '--java-mem-size=16G -outformat HTML'.format(BAMQCRNA, ANNOTATION)
        )
        p1 = exec_command(cmd, detach=True)

        cmd = (
            '{} -bam sample_final.bam --genome-gc-distr HUMAN -nt {} '
    ...
    if 'variant' in STEPS:
        ...
        cmd = '{} mpileup -C 50 -B -q 1 -Q 15 -f {} sample_final.bam > sample.pileup'.format(
            SAMTOOLS, GENOME
        )
        p1 = exec_command(cmd, detach=True)
        ...
        cmd = (
            '{} HaplotypeCaller --reference {} --input sample_final.bam --output haplotypecaller.vcf '
        ...
        cmd = (
            '{} -T {} --primary --ignoreDup -O -C -t exon '
            '-g gene_name -a {} -o gene.counts sample_dedup.bam'.format(
                FEATURECOUNTS, THREADS, ANNOTATION
            )
        )
        ...
        cmd = (
            '{} mpileup2cns sample.pileup --variants 0 --min-coverage 2 --min-reads2 1 --output-vcf 1 '
            '--min-var-freq 0.01 --min-avg-qual 15 --p-value 0.99 --strand-filter 1 > varscan.vcf'.format(
                VARSCAN
            )
        )
    ...
    if 'filter' in STEPS:
        ...
        cmd = (
            '{} VariantFiltration --reference {} --variant haplotypecaller.vcf --window 35 --cluster 3 --filter-name "FS" '
        ...
        cmd = (
            '{} -T CombineVariants -R {} -V:varscan varscan_filtered.vcf '
        ...
        cmd = 'sed -i \'s/{}.HaplotypeCaller/HaplotypeCaller/g\' combined_calls.vcf'.format(
            SAMPLEID
        )
        exec_command(cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--genome', help='Path to the reference genome FASTA file', required=True)
    parser.add_argument(
        '--genome-star',
        help='Path to the reference genome STAR index folder',
        required=False,
        default='',
    )
    parser.add_argument(
        '--genome-ref', help='Path to the reference genome GTF file', required=False, default=''
    )
    parser.add_argument(
        '--sample', help='Name of the sample/experiment. Default is sample', default='sample'
    )
    parser.add_argument(
        '--outdir', help='Path to the folder where to put output files', required=True
    )
    parser.add_argument(
        '--known1',
    ...
    parser.add_argument(
        '--known2', help='Path to the file with 1000G phase indels (GATK bundle)', required=True
    )
    parser.add_argument(
        '--snpsites', help='Path to the file with the SNP sites (GATK bundle)', required=True
    )
    parser.add_argument(
        '--vep-db',
        type=str,
