...
Record_INFO = namedtuple(
    'INFO',
    'Allele Consequence SYMBOL Gene Feature_type Feature BIOTYPE \
...
class Variant:
    ...
    def __str__(self):
        return '{}:{} {}>{} {} {}'.format(
            self.chrom, self.start, self.ref, self.alt, self.type, self.status
        )
...
def epitopes(record, info, ens_data):
    ...
    if 'missense' in funcensGene or 'frame' in funcensGene:
        ...
        if chrom in allowed_contigs:
            ...
            pos, flags, wtmer, mutmer = create_epitope_varcode(
                chrom, record.POS, record.REF, info.Allele, ens_data, transcript
            )
            epitopes.append(
                Epitope(transcript, gene, funcensGene, mut_dna, mut_aa, flags, wtmer, mutmer)
            )
        else:
...
def filter_variants_rna(
    file, tumor_coverage, tumor_var_depth, tumor_var_freq, num_callers, ensembl_version
):
    """
    ...
    for record in reader:
        for info in record.INFO['CSQ']:
            ...
            avsnp150 = (
                record_INFO.Existing_variation.split('&')[0]
                if 'rs' in record_INFO.Existing_variation
                else 'NA'
            )
            gnomad_AF = record_INFO.gnomAD_AF if record_INFO.gnomAD_AF != '' else 'NA'
            cosm_count = record_INFO.Existing_variation.count('COSV')
            cosmic70 = (
                ';'.join(record_INFO.Existing_variation.split('&')[-cosm_count::])
                if cosm_count > 0
                else 'NA'
            )
            ...
            if has_func_ens:
                ...
                try:
                    if 'HaplotypeCaller' in called and 'PASS' in record.FILTER:
                        ...
                        tumor_VAF = (
                            np.around(tumor_AD / float(tumor_DP) * 100, 3)
                            if tumor_DP > 0.0
                            else 0.0
                        )
                        if (
                            tumor_DP >= tumor_coverage
                            and tumor_VAF >= tumor_var_freq
                            and tumor_AD >= tumor_var_depth
                        ):
                            pass_variants += 1
                        filtered['HaplotypeCaller'] = '{};{};{}'.format(
                ...
                variant.callers = '|'.join(
                    ['{}:{}'.format(key, value) for key, value in filtered.items()]
                )
    ...
    for record in reader:
        for info in record.INFO['CSQ']:
            ...
            if has_func_ens:
                ...
                try:
                    if (
                        'NORMAL.mutect' in called
                        and 'TUMOR.mutect' in called
                        and 'PASS' in record.FILTER
                    ):
                        ...
                        tumor_normal_ratio = (
                            tumor_VAF / normal_VAF if normal_VAF != 0 else t2n_ratio
                        )
                        if (
                            normal_DP >= normal_coverage
                            and tumor_DP >= tumor_coverage
                            and tumor_VAF >= tumor_var_freq
                            and tumor_AD >= tumor_var_depth
                            and normal_VAF <= normal_var_freq
                            and tumor_normal_ratio >= t2n_ratio
                        ):
                            pass_snp += 1
                        filtered['mutect'] = '{};{};{};{};{};{}'.format(
                    ...
                    if 'NORMAL.somaticsniper' in called and 'TUMOR.somaticsniper' in called:
                        ...
                        normal_VAF = (
                            np.around((normal_AD / float(normal_DP)) * 100, 3)
                            if normal_DP > 0.0
                            else 0.0
                        )
                        ...
                        if (
                            ...
                            pass_snp += 1
                        if is_somatic:
                            filtered['somaticsniper'] = '{};{};{};{};{};{}'.format(
                                normal_DP, normal_AD, normal_VAF, tumor_DP, tumor_AD, tumor_VAF
                            )

                    if (
                        ('NORMAL.varscan' in called and 'TUMOR.varscan' in called)
                        or ('NORMAL.varscan_indel' in called and 'TUMOR.varscan_indel' in called)
                        and 'PASS' in record.FILTER
                        and 'SOMATIC' in record.INFO
                    ):
                        ...
                        if (
                            ...
                            if 'indel' in label_index:
                                ...
                                pass_snp += 1
                        filtered[label_index] = '{};{};{};{};{};{}'.format(
                            normal_DP, normal_AD, normal_VAF, tumor_DP, tumor_AD, tumor_VAF
                        )
                    if (
                        'NORMAL.strelka' in called
                        and 'TUMOR.strelka' in called
                        and 'PASS' in record.FILTER
                    ):
                        ...
                        normal_VAF = (
                            np.around((normal_AD2 / float(normal_DP)) * 100, 3)
                            if normal_DP > 0.0
                            else 0.0
                        )
                        ...
                        if (
                            ...
                            and tumor_DP >= tumor_coverage
                            and tumor_VAF >= tumor_var_freq
                            and tumor_AD2 >= tumor_var_depth
                            and normal_VAF <= normal_var_freq
                            and tumor_normal_ratio >= t2n_ratio
                        ):
                            pass_snp += 1
                        filtered['strelka'] = '{};{};{};{};{};{}'.format(
                            normal_DP, normal_AD2, normal_VAF, tumor_DP, tumor_AD2, tumor_VAF
                        )
                    if (
                        'NORMAL.strelka_indel' in called
                        and 'TUMOR.strelka_indel' in called
                        and 'PASS' in record.FILTER
                    ):
