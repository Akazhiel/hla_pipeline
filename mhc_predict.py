...
def main(hla, overlap_final, alleles_file, mode, results, results_filter, cutoff):
        ...
        with open('protein_sequences_wt.fasta', 'w') as fwrite_wt:
            with open(overlap_final) as fread:
                ...
                for line in lines:
                    ...
                    pass_germline = (
                        int(columns[header.index('Number of RNA samples (passing)')]) > 0
                    )
                    if (
                        (mode == "both" and pass_somatic and pass_germline)
                        or (mode == "dna" and pass_somatic)
                        or (mode == "rna" and pass_germline)
                        or (mode == "either" and (pass_somatic or pass_germline))
                    ):
                        protein_name = '{}_{}_{}'.format(
                            ''.join(columns[header.index('Variant key')].split()),
                        ...
                        if (
                            protein_seq_mu != '-'
                            and '.' not in protein_seq_mu
                            and protein_seq_mu not in added_proteins_mu
                        ):
                        ...
                        if (
                            protein_seq_wt != '-'
                            and '.' not in protein_seq_wt
                            and protein_seq_wt not in added_proteins_wt
                        ):
    ...
    cmd = (
        'mhcflurry-predict-scan protein_sequences_wt.fasta --alleles {} '
        '--no-throw --results-{} {} --out predictions_wt.csv --peptide-lengths 8-12'.format(
            ' '.join(filtered_hla), results, results_filter
        )
    )
    exec_command(cmd)
