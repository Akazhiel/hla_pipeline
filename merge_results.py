...
def main(
    ...
    header_final = (
        'Variant key\tDBsnp ID\tGnomad MAF\tCosmic ID\tDNA samples (passing)\tNumber of DNA samples (passing)\t'
    ...
    for key, value in variant_dict.items():
        ...
        rna_name_pass = set(
            [name for variant, name in value if variant.type == 'rna' and variant.status]
        )
        rna_name_fail = set(
            [name for variant, name in value if variant.type == 'rna' and not variant.status]
        )
        rna_callers = ';'.join(
            set(
                [
                    '{}:({})'.format(name, variant.callers)
                    for variant, name in value
                    if variant.type == 'rna'
                ]
            )
        )
        dna_name_pass = set(
            [name for variant, name in value if variant.type == 'dna' and variant.status]
        )
        dna_name_fail = set(
            [name for variant, name in value if variant.type == 'dna' and not variant.status]
        )
        dna_callers = ';'.join(
            set(
                [
        ...
        for _, epitopes in epitopes_dict.items():
            ...
            if gene is not None and len(counts_dict) > 0:
                    ...
                    try:
                        ...
                        gene_locus.append(
                            '{}:({})'.format(
                                name,
                                ';'.join(
                                    [gene, str(gene_count), str(gene_mean), str(gene_percentile)]
                                ),
                            )
                        )
                    except KeyError:
                ...
                gene_locus = ["-"]
            effect = ';'.join(
                set(['{}_{}_{}'.format(e.func, e.gene, e.transcript) for e in epitopes])
            )
